...
function readTodayUseMoney(uid) {
    firebaseRootRef.child("records").child(uid).orderByChild("date").equalTo(today).on("value", snapshot => {
...
function readRecordsTable(uid) {
    let html = ``;
    firebaseRootRef.child("records").child(uid).limitToLast(10).on("child_added", (snapshot) => {
...
function getLastBalance() {
    firebaseAuth.onAuthStateChanged((user) => {
        if (user) {
            // User is signed in.
            let uid = user.uid;
            firebaseRootRef.child("records").child(uid).limitToLast(1).on("child_added", (snapshot) => {
